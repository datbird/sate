...
// Ported verbatim from PocketBase pb_hooks/functions.js (the PocketBase-specific key encryption and
// DB provider lookup are NOT here — keys come from the Secrets port, provider config from DataStore).
...
export const PROMPTS: Record<AIFunction, PromptDef> = {
...
export function normalizeNutrition(obj: any): NutritionResult {
  const items: NutritionItem[] = (Array.isArray(obj.items) ? obj.items : []).map((it: any) => ({
  ...
  return { items, total, note: String(obj.note || "") };
}
...
export function normalizeActivity(obj: any): ActivityResult {
  const items: ActivityItem[] = (Array.isArray(obj.items) ? obj.items : []).map((it: any) => ({
