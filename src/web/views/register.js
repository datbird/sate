...
//   • Cloud  (prominent, pre-selected, "Recommended" badge + 30-day free-trial banner) — the managed
//     edition where AI "just works" when entitled. This is the primary CTA.
//   • Hosted (visible but visually de-emphasized, no badge) — the self-hosted edition: BYOAI, run
//     your own Docker + your own AI keys. A clear .ob-warn spells out that this is NOT the managed
//     experience, and choosing it requires a deliberate confirmation before we commit.
//
// NAMING: user-facing copy says Cloud (managed) and Hosted (self-hosted). The `edition` VALUES on the
// wire stay "hosted" (= Cloud) and "selfhost" — they are persisted on the profile and tied to the live
// sate_hosted / sate_selfhost SKUs, so they must not be renamed. Display text only below.
//
...
// stage ("pick" | "confirm"). Cloud is the default so the primary CTA is always live. We build/remove
...
function draw() {
  ...
  const cloudOn = sel === "hosted"; // wire value "hosted" IS the Cloud edition
  ...
  body.innerHTML =
    ...
    '<div class="plan-cards">' +
      `<button type="button" class="plan-card${cloudOn ? " on" : ""}" data-ed="hosted">` +
        `<span class="plan-name">${esc(app)} Cloud <span class="plan-badge">Recommended</span></span>` +
      ...
      `<button type="button" class="plan-card${selfOn ? " on" : ""}" data-ed="selfhost">` +
        `<span class="plan-name">${esc(app)} Hosted</span>` +
      ...
      '</button>' +
    '</div>' +
    (cloudOn && perm ? `<div class="trial-banner">✨ ${accessNote()}</div>` : '') +
    (cloudOn && !perm
      ...
      : '') +
    (selfOn
      ? '<div class="ob-warn"><b>Hosted means you run it yourself — it is not the managed experience.</b> You’ll run the Sate Docker container on your own machine and supply your own AI provider keys (OpenAI, Gemini, etc.). There is no managed cloud, no automatic updates, and no built-in AI — it’s all on you.</div>'
      : '') +
    '<div class="ob-nav"><span></span>' +
      `<button type="button" class="primary" id="regNext">${
        selfOn ? "Continue with Hosted" : perm ? "Get started" : "Start free trial"
      }</button>` +
    '</div>';
...
function drawConfirm(body) {
  body.innerHTML =
    '<h2 style="margin-top:6px">Confirm Hosted (self-hosted)</h2>' +
    ...
    'There is no managed cloud, no automatic updates, and no built-in AI — it’s all on you. Cloud is the recommended, zero-setup option.</div>' +
