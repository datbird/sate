...
//   Hosted (PocketBase/goja):  require(`${__hooks}/shared/nutrition.js`)
//   Cloud  (Node/TypeScript):  import * as nutrition from "../shared/nutrition.js"
//
// Authored as CommonJS in goja-safe ES2015 (goja has no ES modules; no `?.`, no `??`, no object
// spread). Types for TS consumers live in the sibling nutrition.d.ts — keep the two in step.
//
// Both the /plan/compute endpoint (numbers the app saves as goals) and the nutritionist AI
// (grounding context) run off this, so the algorithm and the AI always agree on the math.
